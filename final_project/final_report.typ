#set page(paper: "a4", margin: (top: 22mm, bottom: 24mm, left: 22mm, right: 22mm))
#set text(font: ("Libertinus Serif", "Times New Roman", "DejaVu Serif"), size: 10pt, lang: "en")
#set par(justify: true, leading: 0.58em, first-line-indent: 1.2em)
...
#align(center)[
  #text(size: 18pt, weight: "bold")[
    Amount- and position-aware analysis of LIN28A binding as a predictor of translational derepression after #emph[Lin28a] knockdown
  ]
...
#figure(
  image("results/figures/fig1_baseline_total_clip_vs_delta_rd.pdf", width: 82%),
  caption: [#strong[Fig. 1 | LIN28A binding amount correlates with translational derepression after #emph[Lin28a] knockdown.] Each point is a gene (n = 8,986 baseline-eligible genes). The x axis shows transcript-level LIN28A CLIP enrichment, log2(CPM_CLIP / CPM_RNA-control), from featureCounts. The y axis shows DeltaRD, log2[(RPF/RNA)_siLin28a / (RPF/RNA)_siLuc]. The red line is an ordinary least-squares fit. Binding and derepression are positively correlated (Pearson r = 0.4637, p < 1e-300; Spearman rho = 0.4578), recapitulating the transcript-level relationship in Cho et al. [4].]
)
...
#figure(
  image("results/figures/fig2_region_composition_summary.pdf", width: 100%),
  caption: [#strong[Fig. 2 | Defining an RNA-normalized regional binding composition.] #strong[a,] Distribution of 5-prime UTR, CDS and 3-prime UTR lengths for representative transcripts. #strong[b,] RNA-control-adjusted binding composition for composition-eligible genes (n = 5,723), where regional enrichment is normalized to a fraction across the three transcript regions. #strong[c,] Mean raw CLIP 5-prime point fractions compared with RNA-adjusted fractions, illustrating why regional RNA-control normalization is needed. #strong[d,] Filtering flow from all count-table genes to the final model set.]
)
...
#figure(
  image("results/figures/fig3_nested_model.pdf", width: 100%),
  caption: [#strong[Fig. 3 | Total binding amount is the larger predictor than regional position.] Nested OLS models were fit on the same 5,723 composition-eligible genes. #strong[a,] Variance explained by M0, M1 and M2. #strong[b,] Incremental variance explained: total binding amount adds DeltaR2 = 0.2142, whereas regional composition adds DeltaR2 = 0.0080 (M1 versus M2 nested F-test, p = 1.2e-14). #strong[c,] Fully standardized M2 coefficients, showing that total binding is the largest term and that the CDS composition term is not positive.]
)
...
1. Viswanathan, S. R., Daley, G. Q. & Gregory, R. I. Selective blockade of microRNA processing by Lin28. #emph[Science] #strong[320], 97-100 (2008). https://doi.org/10.1126/science.1154040

2. Heo, I. #emph[et al.] Lin28 mediates the terminal uridylation of let-7 precursor MicroRNA. #emph[Mol. Cell] #strong[32], 276-284 (2008). https://doi.org/10.1016/j.molcel.2008.09.014

3. Wilbert, M. L. #emph[et al.] LIN28 binds messenger RNAs at GGAGA motifs and regulates splicing factor abundance. #emph[Mol. Cell] #strong[48], 195-206 (2012). https://doi.org/10.1016/j.molcel.2012.08.004

4. Cho, J. #emph[et al.] LIN28A is a suppressor of ER-associated translation in embryonic stem cells. #emph[Cell] #strong[151], 765-777 (2012). https://doi.org/10.1016/j.cell.2012.10.019

5. Ingolia, N. T., Ghaemmaghami, S., Newman, J. R. S. & Weissman, J. S. Genome-wide analysis in vivo of translation with nucleotide resolution using ribosome profiling. #emph[Science] #strong[324], 218-223 (2009). https://doi.org/10.1126/science.1168978

6. Licatalosi, D. D. #emph[et al.] HITS-CLIP yields genome-wide insights into brain alternative RNA processing. #emph[Nature] #strong[456], 464-469 (2008). https://doi.org/10.1038/nature07488

7. Zhang, C. & Darnell, R. B. Mapping in vivo protein-RNA interactions at single-nucleotide resolution from HITS-CLIP data. #emph[Nat. Biotechnol.] #strong[29], 607-614 (2011). https://doi.org/10.1038/nbt.1873

= Methods

== Data source and study context

This project reanalysed the local final-project datapack derived from Cho et al. [4], which contains LIN28A CLIP-seq, RNA-seq and ribosome-footprinting libraries from mouse embryonic stem cells. Ribosome profiling was originally introduced as a nucleotide-resolution method for measuring translation in vivo [5], and HITS-CLIP established a sequencing-based framework for mapping protein-RNA interactions in living tissue [6]. The original Cho study reported that LIN28A binds many mature mRNAs and that LIN28A-bound transcripts show increased ribosome occupancy after #emph[Lin28a] knockdown. The present analysis used the local BAM files, the GENCODE GTF annotation and the week 1 featureCounts table supplied with the course project. No external data were downloaded for the course-project analysis.
